...
RSpec.describe GraphManager do
  let(:graph) { Graph.new } # Initialize a graph instance
  let(:graph_manager) { GraphManager.new(graph, 1) } # Set batch_size to 1 for immediate processing

  describe '#process_log' do
    it 'adds a node to the graph when receiving an add_node event' do
      log = '{"event": "add_node", "id": "N5"}'
      expect { graph_manager.process_log(log) }.to change { graph_manager.graph.nodes.size }.by(1)
    end

    it 'creates an edge between nodes when receiving a transition event' do
  ...
  describe '#record_metrics' do
    it 'records metrics over time' do
      ...
      latest_metrics = graph_manager.metrics_history.last
      expect(latest_metrics[:total_nodes]).to eq(1)
      expect(latest_metrics[:total_edges]).to eq(0)
      expect(latest_metrics[:avg_connections]).to eq(0.0)
    end
  end
